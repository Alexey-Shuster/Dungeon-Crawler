...
TEST_F(ResponseSenderTest, AuthSendsWelcome) {
    ...
    auto decoded = deserializeBufferToMessage(ByteBuffer(std::move(session->captured_data_)));
...
TEST_F(ResponseSenderTest, AuthHandlesMultiplePlayers) {
    ...
    auto decoded1 = deserializeBufferToMessage(ByteBuffer(std::move(session1->captured_data_)));
    ...
    auto decoded2 = deserializeBufferToMessage(ByteBuffer(std::move(session2->captured_data_)));
