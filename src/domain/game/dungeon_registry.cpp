...
bool DungeonRegistry::addDungeon(map::GameMap game_map, std::vector<PlayerId> player_ids) {
    if (player_ids.empty()) {
    ...
    std::unordered_set<PlayerId, StrongIdHash<PlayerId>> unique_check(player_ids.begin(), player_ids.end());
...
const std::unordered_map<GameId, std::shared_ptr<Dungeon>, StrongIdHash<GameId>>& DungeonRegistry::getAllDungeons() {
    return dungeons_;
}
