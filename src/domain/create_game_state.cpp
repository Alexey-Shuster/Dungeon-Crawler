...
DungeonSnapshot createGameStateDTO(const dungeon::DungeonState& state) {
    ...
    auto addEntity = [&](auto id, const auto& entity, uint8_t type) {
        snapshot.entities.push_back(
            {.type = type,
             .state = static_cast<uint8_t>(entity.isAlive() ? entity::EntityState::Alive : entity::EntityState::Dead),
             .id = id.value,
    ...
    for (const auto& [id, player] : state.players) {
        addEntity(id, player, static_cast<uint8_t>(entity::EntityType::Player));
    }
    for (const auto& [id, monster] : state.monsters) {
        addEntity(id, monster, static_cast<uint8_t>(entity::EntityType::Monster));
    }
