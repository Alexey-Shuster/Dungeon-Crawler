...
std::shared_ptr<Session> Session::create(boost::asio::ip::tcp::socket socket,
    ...
    struct CreateMakeShared : Session {
        CreateMakeShared(boost::asio::ip::tcp::socket socket, events::EventBus& eventBus, SessionId sid) :
...
void Session::start() {
    ...
    eventBus_.publish(events::ClientConnectedEvent{shared_from_this()});
}

void Session::send(MessageData&& raw_message) {
    boost::asio::dispatch(strand_, [this, self = shared_from_this(), msg = std::move(raw_message)]() {
        ...
        MessageData encoded = FrameCodec::encodeFrame(std::move(msg));
        if (encoded.empty()) {
            LOG_ERROR(std::format("[Session {}] encoding failed", sessionId_.value));
            return;
        }

        auto msg_ptr = std::make_shared<MessageData>(std::move(encoded));
...
void Session::handleDisconnect() {
    ...
    eventBus_.publish(events::ClientDisconnectedEvent{shared_from_this()});
}

Session::Session(boost::asio::ip::tcp::socket socket, events::EventBus& eventBus, SessionId sid) :
    socket_(std::move(socket)), strand_(socket_.get_executor()), eventBus_(eventBus), sessionId_(sid) {
...
void Session::processMessage(MessageData&& raw_msg) const {
    LOG_INFO(std::format("[Session {}] sending RawMessageReceivedEvent. Included message size={} bytes",
                         sessionId_.value,
                         raw_msg.size()));
    eventBus_.publish(events::RawMessageReceivedEvent{getSessionId(), Message{std::move(raw_msg)}});
}
}  // namespace network
