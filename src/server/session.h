...
public:
     ...
     */
    virtual void send(MessageData&& raw_message);
...
protected:
    Session(boost::asio::ip::tcp::socket socket, events::EventBus& eventBus, SessionId sid);
...
private:
    ...
    void processMessage(MessageData&& raw_msg) const;
...
private:
    ...
    std::deque<std::shared_ptr<MessageData>> write_queue_;  ///< Outgoing message queue
