...
public:
    GameLoop(boost::asio::io_context& ioc, std::shared_ptr<events::EventBus> bus, std::chrono::milliseconds rate) :
        io_context_(ioc), event_bus_(std::move(bus)), tick_rate_(rate), timer_(io_context_) {
        if (!event_bus_) {
...
private:
    ...
    void scheduleTick() {
        ...
        timer_.async_wait([self = shared_from_this(), this](const boost::system::error_code& ec) {
            ...
            events::GameTickEvent event(now);
