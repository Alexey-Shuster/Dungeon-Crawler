...
std::shared_ptr<LobbyManager> LobbyManager::create(boost::asio::io_context& io_context,
    ...
    struct EnableMakeShared : LobbyManager {
        EnableMakeShared(boost::asio::io_context& ioc,
                         std::shared_ptr<events::EventBus> eb,
                         std::shared_ptr<LobbyRegistry> lr) : LobbyManager(ioc, std::move(eb), std::move(lr)) {}
    };
...
LobbyManager::LobbyManager(boost::asio::io_context& io_context,
                           std::shared_ptr<events::EventBus> eventBus,
                           std::shared_ptr<LobbyRegistry> lobbyRegistry) :
    strand_(boost::asio::make_strand(io_context)), event_bus_(std::move(eventBus)),
    lobby_registry_(std::move(lobbyRegistry)) {}

void LobbyManager::Initialize() {
    // auto self = shared_from_this();

    subscribeWeak<events::CreateLobbyRequestEvent>(&LobbyManager::onCreateLobbyRequest);

    subscribeWeak<events::JoinLobbyRequestEvent>(&LobbyManager::onJoinLobbyRequest);

    subscribeWeak<events::LeaveLobbyRequestEvent>(&LobbyManager::onLeaveLobbyRequest);

    subscribeWeak<events::PlayerReadyRequestEvent>(&LobbyManager::onPlayerReadyRequest);

    subscribeWeak<events::ListLobbiesRequestEvent>(&LobbyManager::onListLobbiesRequest);
}

void LobbyManager::onCreateLobbyRequest(const events::CreateLobbyRequestEvent& event) {
    ...
    if (lobby_registry_->isPlayerInLobby(event.player_id)) {
        ...
        publishEvent<events::LobbyCreationFailedResponseEvent>(event.player_id);
        return;
    }
    ...
    publishEvent<events::LobbyCreatedResponseEvent>(event.player_id, newId);
}

void LobbyManager::onJoinLobbyRequest(const events::JoinLobbyRequestEvent& event) {
    auto lobby = lobby_registry_->findLobby(event.lobby_id);
    if (!lobby) {
        publishEvent<events::JoinLobbyNotExistsResponseEvent>(event.player_id);
        return;
    }
    if (lobby->isFull()) {
        publishEvent<events::JoinLobbyFullResponseEvent>(event.player_id);
        return;
    }

    auto success = lobby_registry_->addPlayerToLobby(event.player_id, event.lobby_id);
    if (success) {
        publishEvent<events::JoinLobbyResponseEvent>(event.player_id);
    } else {
        LOG_INFO(
            std::format("Join request failed for player {} to lobby {}", event.player_id.value, event.lobby_id.value));
        publishEvent<events::JoinLobbyFailedResponseEvent>(event.player_id);
    }
}

void LobbyManager::onLeaveLobbyRequest(const events::LeaveLobbyRequestEvent& event) {
    LobbyId lobbyId{0};
    auto result = getLobbyForPlayer(event.player_id, lobbyId);

    if (result.error == LobbyLookupError::PlayerNotInLobby) {
        publishEvent<events::LeaveLobbyNotConsistsInLobbyResponseEvent>(event.player_id);
        return;
    }
    if (result.error == LobbyLookupError::StaleMapping) {
        publishEvent<events::LeaveLobbyFailedResponseEvent>(event.player_id);
        return;
    }

    bool removed = lobby_registry_->removePlayerFromLobby(event.player_id);
    if (removed) {
        publishEvent<events::LeaveLobbyResponseEvent>(event.player_id);
    } else {
...
void LobbyManager::onPlayerReadyRequest(const events::PlayerReadyRequestEvent& event) {
    LobbyId lobbyId{0};
    auto result = getLobbyForPlayer(event.player_id, lobbyId);
    ...
    if (result.error == LobbyLookupError::PlayerNotInLobby || result.error == LobbyLookupError::StaleMapping) {
        publishEvent<events::PlayerReadyFailedResponseEvent>(event.player_id);
        return;
    }

    if (result.lobby->setReady(event.player_id, event.is_ready)) {
        publishEvent<events::PlayerReadyResponseEvent>(event.player_id);
    } else {
...
void LobbyManager::onListLobbiesRequest(const events::ListLobbiesRequestEvent& event) {
    ...
    publishEvent<events::ListLobbiesResponseEvent>(event.player_id, lobbies_ids);
}
