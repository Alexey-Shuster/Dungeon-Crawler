...
void GameManager::Initialize() {
    subscribeWeak<events::StartGameRequestEvent>(&GameManager::onStartGameRequestEvent);
    subscribeWeak<events::GameTickEvent>(&GameManager::onGameTickEvent);
    subscribeWeak<events::MoveRequestEvent>(&GameManager::onMoveRequestEvent);
    subscribeWeak<events::AtackRequestEvent>(&GameManager::onAttackRequestEvent);
}
void GameManager::onStartGameRequestEvent(const events::StartGameRequestEvent& event) {
    ...
    if (auto lobby_id_opt = lobby_registry_->getPlayerLobby(player_id); lobby_id_opt.has_value()) {
        ...
        if (lobby) {
            if (dungeon_registry_.addDungeon(map::GameMap{}, lobby->getAllPlayers())) {
...
void GameManager::onGameTickEvent(const events::GameTickEvent& event) {
    std::vector<GameId> dungeons_to_remove;
    auto milliseconds =
    ...
    for (auto& [game_id, dungeon] : dungeon_registry_.getAllDungeons()) {
        ...
        if (!dungeon_state.has_value()) {
            ...
            for (const auto& pid : dungeon->getPlayers()) {
                postEvent<events::GameOverEvent>(pid);
        ...
        postEvent<events::GameStateUpdateEvent>(game_id, dungeon_state.value());
